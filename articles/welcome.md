# Welcome to BMCN,
## *The Official Newsletter of BendersMC*

---

### BMCN Introduction
*BMCN stands for The BendersMC Newsletter. BMCN is the current leading newsletter, which has an official Discord and YouTube channel.*
  BMCN has a dedicated staff team from multiple major clans that are enemies to allies with each other. BMCN's goal is to serve the community of BendersMC by giving them unbiased news reports, cool and fun events, and so much more. BMCN has been around since late 2024, when it was at its peak.
  
#### Our Staff
*BMCN's staff team has been around, writing and editing articles since BMCN was founded.*

**Founder of BMCH-BMCN:**
  - *JackTheFounder*

##### Human Resources

  - CEOs: *JackTheFounder, Eulm*
  - General Manager: *Open Spot*
  - Head News Reporter: *Open Spot*
  - HR Reporters: *ForwardHarbor*
  - HR Editors: *Aayan, Chromium*
  - HR Photographers: *Sans, Blamster858*
  - HR Videographers: *Lobster*
  
##### Regular Staff
  
  - News Anchors: *Fire, Youb19*
  - Reporters: *Issak, Redgamer, Luna*
  - Journalists: *Lupus, Masta*
  - Editors: *Open Spots*
  - Photographers: *Silver*
  - Designers: *Open Spots*
  - Moderators: *The Coolest Man*

##### Former HR Staff

  - CEOs: *yJackie*
  - HR Manager: *FishyMC*
  - HR Reporters: *None*
  - HR Editors: *None*
  - HR Photographers: *None*
  - HR Videographers: *None*
